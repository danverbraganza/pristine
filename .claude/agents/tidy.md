...
- **Test-helper proliferation** - Multiple drain/wait/assertion idioms in the same test module that should share one helper. Each new bead tends to add its own variant; old variants are rarely folded back. Inspect test modules in the files touched by recent beads.

## Actions you may take
- File beads to fix high-priority problems that you identify using `br create`. Bead IDs are auto-generated by `br create`; do not pass `--id`.
